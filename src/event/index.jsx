...
import { doubles_rating_changes, init_doubles } from "../doubles-ratings"
...
export function DoublesTournamentCard({ event_id, close, showPlayer }) {
  const tourney = tournamentById.get(event_id)
  const [matches, setMatches] = useState([])

  useEffect(() => {
    init_doubles.then((db) => {
      return db.getAllFromIndex('doubles', 'event_id', event_id)
    }).then((matches) => {
      for (let i = matches.length - 1; i >= 0; i--) {
        matches[i].rc = doubles_rating_changes.get(matches[i].id)
        matches[i].end = Date.parse(tournamentById.get(matches[i].event_id).EndDateTime)
      }
      matches.sort(sortStartStage)
      setMatches(matches)
  ...
  return (
    <div className="bracket-card card">
      <div className="card-header">
        {`[${tourney.EndDate}]`} {tourney.EventName} (Doubles)
        <div className="card-close" onClick={close}>x</div>
      </div>
      <div className="card-content">
        {matches.map(m => {
          return <DoublesMatchRow key={m.id} m={m} showPlayer={showPlayer} />
        })}
      </div>
    </div>
  )
...
export function DoublesMatchRow({ m, showPlayer }) {
  const scores = m.scores.split(',').map(x => x.split('-'))
  const change1 = Math.round(m.rc.new_r1_1 - m.rc.r1_1)
  const change2 = Math.round(m.rc.new_r2_1 - m.rc.r2_1)
  ...
  return (
    <div key={m.id} className="match-row event-match-row">
      ...
      <div className={`match-rating-change ${change1 < 0 ? 'match-loss' : ''}`}>{change1}</div>
      <div className="match-ratings">
        {Math.floor(m.rc.r1_1)}
      </div>
      <div className="match-opponent">
        <span className="player-link" data-playerid={m.a_id} onClick={showPlayer}>{playerById.get(m.a_id)?.name}</span>
        {" / "}
        <span className="player-link" data-playerid={m.b_id} onClick={showPlayer}>{playerById.get(m.b_id)?.name}</span>
      </div>
      <div className="match-res">{m.res_a} - {m.res_x}</div>
      <div className="match-opponent">
        <span className="player-link" data-playerid={m.x_id} onClick={showPlayer}>{playerById.get(m.x_id)?.name}</span>
        {" / "}
        <span className="player-link" data-playerid={m.y_id} onClick={showPlayer}>{playerById.get(m.y_id)?.name}</span>
      </div>
      <div className="match-ratings">
        {Math.floor(m.rc.r2_1)}
      </div>
      <div className={`match-rating-change ${change2 < 0 ? 'match-loss' : ''}`}>{change2}</div>
      <div className="match-scores">
        {scores.map((set, i) =>
